...
class CameraRollPicker extends Component {
  ...
  selectImage(image, index) {
    ...
    if (indexInSelected >= 0) {
      ...
      console.warn('On Select 1', selectedImages, selected)
    } else {
      if (selectSingleItem) {
        selected = [index]
        selectedImages = [image]
        console.warn('On Select 2', selectedImages, selected)
      } else {
        if (selected.length < maximum) {
          selected.push(index)
          selectedImages.push(image)
          console.warn('On Select 3', selectedImages, selected)
        } else {
          // maximum
  ...
  render() {
    ...
    const flatListOrEmptyText = images.length > 0 ? (
      <FlatList
        ...
        renderItem={({ item, index }) => this.renderImage.bind(this)(item, index)}
